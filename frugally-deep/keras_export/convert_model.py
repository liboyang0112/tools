#!/usr/bin/env python3
"""Convert a tensorflow.Keras model to frugally-deep format.
"""
...
def get_layer_input_shape_shape5(layer):
    """Convert a tensorflow.keras shape to an fdeep shape"""
...
def gen_test_data(model):
    ...
    def generate_input_data(input_layer):
        ...
        if input_layer._outbound_nodes and isinstance(
                input_layer._outbound_nodes[0].outbound_layer, tensorflow.keras.layers.Embedding):
...
def main():
    ...
    usage = 'usage: [tensorflow.Keras model in HDF5 format] [output path]'
